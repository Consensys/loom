...
func ensurePolynomialsAreInNormalLayout(P map[string]*Polynomial) error {
	for name, k := range P {
		if k.IsConstant() {
			continue
		}
		if k.EP.Layout != Normal {
			return fmt.Errorf("all polynomials must be in Normal layout, %s is not in Normal layout", name)
		}
	}
	return nil
}
