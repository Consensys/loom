...
// 1. GP*E2-GP*E1=0
// 2. GP_Shifted[0]=1
func BuildGrandProductRelation(E1, E2 sym.Expr, GP string, N int) []Relation {
	...
	// 2. GP[0]=1
	boundaryRelation := BuildLocalRelation(sym.NewCommittedColumn(GP), sym.NewConst(koalabear.One()), 0, N)
...
func BuildGrandSumRelations(M, E sym.Expr, grandSum string, N int) []Relation {
...
func BuildLocalRelation(E, M sym.Expr, i int, N int) Relation {
	lagrangeID := proveractions.GetLagrangeID(i, N)
	localRelation := sym.NewComputableColumn(lagrangeID).Mul(E.Sub(M))
	return localRelation
}

// BuildCorrectConstructionRelation adds a constraint idRes - E=0, to ensure that IdRes is correcly
// constructed with E
func BuildCorrectConstructionRelation(E sym.Expr, IdRes string) Relation {
...
func BuildFilteredAccPolynomialRelation(E, F, alpha sym.Expr, R string, N int) []Relation {

	// 1. R[0] = F[0]*E[0]
	boundaryRelation := BuildLocalRelation(sym.NewCommittedColumn(R), E.Mul(F), 0, N)
	...
	recurrenceRelation := sym.NewCommittedColumn(R).Sub(path1).Mul(sym.NewConst(one).Sub(lagrange0)) // (R[i] - (F[i]*(α*R[i-1]+E[i]) + (1-F[i])R[i-1])) * (1 - lagrange0)
