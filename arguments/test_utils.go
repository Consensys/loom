...
func sanityCheck(proverRunTime *prover.Prover, constraints []constraint.Relation, N int, t *testing.T) {
	err := BruteForceChecker(proverRunTime.Trace, constraints, N)
	if err != nil {
		t.Fatal(err)
	}
	err = QuotientChecker(proverRunTime.Trace, constraints, N)
	if err != nil {
		t.Fatal(err)
	}
}
...
func BruteForceChecker(T trace.Trace, constraints []constraint.Relation, N int) error {
...
func QuotientChecker(T trace.Trace, constraints []constraint.Relation, N int) error {
