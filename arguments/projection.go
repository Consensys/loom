...
func ProjectionExpr(system *cs.Builder, A, B, F1, F2 expr.Expr) error {
	...
	system.RegisterDerivationStep(depsAlpha, []string{_alpha}, proveractions.NewIDStepContext(proveractions.FIAT_SHAMIR))

	// 3. create the filtered acc polynomials
	inputsFA := []expr.Expr{A, F1, alpha}
	system.RegisterDerivationStep(inputsFA, []string{idAccFA}, proveractions.NewIDStepContext(proveractions.FITLERED_ACC_POLY))
	inputsFB := []expr.Expr{B, F2, alpha}
	system.RegisterDerivationStep(inputsFB, []string{idAccFB}, proveractions.NewIDStepContext(proveractions.FITLERED_ACC_POLY))
