...
// M[i] = #{j | j S[j]=T[i]}
func ComputeMultiplicity(trace trace.Trace, proof *Proof, mu *sync.Mutex, E []sym.Expr, M []string) error {
	if len(E) != 2 {
		return fmt.Errorf("len(E)=%d, expected 2", len(E))
	}
	if len(M) != 1 {
...
func ComputeFilteredAccPolynomial(trace trace.Trace, proof *Proof, mu *sync.Mutex, E []sym.Expr, output []string) error {
